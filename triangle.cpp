...
float Triangle::get_area() {
    return glm::length(glm::cross(v[1] -v[0], v[2] - v[1])) * 0.5f;
}
...
Intersection Triangle::get_interaction(const Ray &ray) {
    ...
    float s1dote1 = glm::dot(s1, e1);
    float t = glm::dot(s2, e2) / s1dote1;
    if (t < -EPSILON) {
        return inter;
    }
    float b1 = glm::dot(s1, s) / s1dote1;
    if (b1 < -EPSILON) {
        return inter;
    }
    float b2 = glm::dot(s2, ray.dir) / s1dote1;
    if (b2 > -EPSILON && (1 - b1 - b2) > - EPSILON) {
        inter.has = true;
        inter.normal = normal;
        inter.obj = this;
        inter.m = m;
        inter.pos = ray.ori + ray.dir * t;
        inter.t = t;
        return inter;
    } else {
        return inter;
...
void Triangle::sample(Intersection& inter, float &pdf) {
    ...
    vec3 p = x1 * v[0] + (1 - x1) * (x2 * v[1] + (1 - x2) * v[2]);
    inter.has = true;
    inter.pos = p;
    inter.m = m;
    inter.normal = normal;
